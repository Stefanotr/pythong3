...
class RhythmView():
    def __init__(self, screen_width, screen_height):
        ...
        try:
            ...
            # Voile noir plus léger pour mieux voir l'image
            self.overlay = pygame.Surface((screen_width, screen_height))
            self.overlay.fill((0, 0, 0))
            self.overlay.set_alpha(100)  # Plus transparent qu'avant
        ...
        self.lane_colors = [
            (255, 20, 147),   # Rose néon (C)
            (0, 255, 255),    # Cyan néon (V)
            (50, 255, 50),    # Vert néon (B)
            (255, 215, 0)     # Or néon (N)
        ]
    ...
    def create_particles(self, x, y, color):
        """Créer des particules d'explosion pour les hits"""
        for _ in range(12):
    ...
    def draw_health_bar(self, screen, x, y, width, height, current, maximum, label, color_good, color_bad, is_boss=False):
        """Dessiner une barre de vie stylée"""
