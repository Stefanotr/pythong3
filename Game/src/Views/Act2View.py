...
class Act2View:
    ...
    def __init__(self, screen, player=None):
        ...
        try:
            ...
            # === CHARACTER VIEWS WILL BE INITIALIZED BELOW ===
            # (This allows combat controller to receive view references for animations)
            
            try:
                # Create character views with specific sprites and larger size for combat visibility
                # Player: Lola Coma with animation sprites - 180x180 for better visibility
                self.player_view = CaracterView(
                ...
                # Boss: Chef de la Sécurité - 180x180 for better visibility
                # Flip horizontal to make boss face opposite direction
                self.boss_view = CaracterView(
                    "Game/Assets/agentdesecuritequisebaisse (1).png",
                    scale=(180, 180),
                    animation_sprites={
                        "attack": "Game/Assets/agentdesecuritequitape.png",
                        "dodge": "Game/Assets/agentdesecuritequisebaisse (1).png",
                        "hit": "Game/Assets/agentdesecuritequisebaisse (1).png"
                    },
                    flip_vertical=False
                )
                ...
                Logger.debug("Act2View.__init__", "Character views created with security chief sprites", 
                           player_img="lola.png", boss_img="agentdesecuritequisebaisse (1).png", sprite_scale="180x180")
            except Exception as e:
                Logger.error("Act2View.__init__", e)
                # Continue even if character views fail
            
            # === REINITIALIZE COMBAT WITH VIEWS ===
            
            try:
                # Recreate combat controller with character views for animations
                self.combat_model = CombatModel(self.johnny, self.security_chief)
                self.combat_controller = CombatController(self.combat_model, self.player_view, self.boss_view)
                self.combat_view = CombatView(self.screen_width, self.screen_height)
                Logger.debug("Act2View.__init__", "Combat system initialized with animations")
            except Exception as e:
                Logger.error("Act2View.__init__", e)
                raise
            ...
            self.phase = "intro"  # "intro", "combat", "rhythm", "rhythm_finished", "finished"
            ...
            while running:
                try:
                    ...
                    for event in pygame.event.get():
                        ...
                        # Rhythm finished events (waiting for continue)
                        elif self.phase == "rhythm_finished":
                            try:
                                if self.rhythm_controller:
                                    self.rhythm_controller.handleInput(event)
                                
                                # Check for manual continue (SPACE)
                                if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                                    if self.rhythm_controller and self.rhythm_controller.continue_pressed:
                                        self.phase = "finished"
                                        running = False
                                        Logger.debug("Act2View.run", "Rhythm finished - SPACE pressed")
                            except Exception as e:
                                Logger.error("Act2View.run", e)
                    ...
                    if self.phase == "intro":
                        ...
                        try:
                            ...
                            if self.rhythm_controller and self.rhythm_controller.song_finished:
                                self.phase = "rhythm_finished"
                                Logger.debug("Act2View.run", "Rhythm song finished, showing end screen")
                        except Exception as e:
                            Logger.error("Act2View.run", e)
                        ...
                        try:
                            ...
                            if self.rhythm_controller and self.rhythm_controller.continue_pressed:
                                self.phase = "finished"
                                Logger.debug("Act2View.run", "Rhythm finished - auto-continue triggered")
                        except Exception as e:
                            Logger.error("Act2View.run", e)
                    ...
                    try:
                        if self.phase == "intro":
                            ...
                            if self.rhythm_view and self.rhythm_model:
                                # Draw end screen with countdown
                                self.rhythm_view.draw(
                                    self.screen, 
                                    self.rhythm_model, 
                                    self.johnny,
    ...
    def initRhythmPhase(self):
        ...
        try:
            ...
            # Count total notes
            self.rhythm_total_notes = len([n for n in self.rhythm_model.notes if n.get("active", True)])
            self.rhythm_notes_completed = 0
