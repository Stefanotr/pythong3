...
class WelcomPageView(PageView):
    ...
    def __init__(self, name, width=800, height=800, RESIZABLE=0, background_image ="Game/Assets/welcomePage.png"):
        ...
        try:
            ...
            try:
                ...
                play_button_controller = ButtonController(self.play_button, "startGame")
            ...
            Logger.debug("WelcomPageView.__init__", "Stage selector initialized", defaultStage=self.selected_stage)
    ...
    def updateButtonPositions(self):
        ...
        try:
            ...
            Logger.debug("WelcomPageView.updateButtonPositions", "Button positions updated")
        except Exception as e:
            Logger.error("WelcomPageView.updateButtonPositions", e)
    ...
    def toggleFullscreen(self):
        ...
        try:
            ...
            if current_flags & pygame.FULLSCREEN:
                ...
                Logger.debug("WelcomPageView.toggleFullscreen", "Switched to WINDOWED mode")
            else:
                ...
                Logger.debug("WelcomPageView.toggleFullscreen", "Switched to FULLSCREEN mode")
            
            self.updateButtonPositions()
        except Exception as e:
            Logger.error("WelcomPageView.toggleFullscreen", e)
        ...
        try:
            for event in events:
                if event.type == pygame.QUIT:
                    Logger.debug("WelcomPageView.handleEvents", "QUIT event received")
                    return False
                ...
                if event.type == pygame.VIDEORESIZE:
                    ...
                    try:
                        ...
                        Logger.error("WelcomPageView.handleEvents", e)
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key in numeric_keys:
                        ...
                        if self.is_admin:
                            ...
                            try:
                                result = self.startGameFlow(self.selected_stage)
                                Logger.debug(
                                    "WelcomPageView.handleEvents",
                                ...
                                if result == GameState.QUIT.value:
                                    Logger.debug("WelcomPageView.handleEvents", "Quit requested during game flow, exiting menu")
                                    return False
                            except Exception as e:
                                Logger.error("WelcomPageView.handleEvents.stage_jump", e)
                        else:
                            Logger.debug(
                                "WelcomPageView.handleEvents",
                    ...
                    if event.key == pygame.K_F11:
                        try:
                            ...
                            Logger.error("WelcomPageView.handleEvents", f"Failed to toggle fullscreen: {e}")
                        continue
                    ...
                    if action == GameState.START_GAME.value:
                        ...
                        try:
                            ...
                            if result == GameState.QUIT.value:
                                ...
                                Logger.debug("WelcomPageView.handleEvents", "Logout requested during game flow, returning to login")
                                return GameState.LOGOUT.value
                        except Exception as e:
                            Logger.error("WelcomPageView.handleEvents", e)
    ...
    def loadPlayerFromProgression(self, progression_data):
        ...
        try:
            ...
            player.setComaRisk(progression_data.get("comaRisk", 0))
            ...
            try:
                laPelle = GuitarFactory.createLaPelle()
            except Exception as e:
                Logger.error("WelcomPageView.loadPlayerFromProgression", f"Failed to load guitar: {e}")
            
            Logger.debug("WelcomPageView.loadPlayerFromProgression", "Player loaded from progression", 
                        username=self.current_user)
            return player
        except Exception as e:
            Logger.error("WelcomPageView.loadPlayerFromProgression", e)
            raise
    
    def savePlayerProgression(self, player, sequence_controller):
        ...
        try:
            ...
            if success:
                ...
                Logger.debug("WelcomPageView.savePlayerProgression", "Failed to save player progression", 
                           username=self.current_user)
            
            return success
        except Exception as e:
            Logger.error("WelcomPageView.savePlayerProgression", e)
            return False
    
    def startGameFlow(self, starting_stage =0):
        ...
        try:
            ...
            if self.music_playing:
                try:
                    ...
                    self.music_playing = False
                    Logger.debug("WelcomPageView.startGameFlow", "Welcome music stopped")
                except Exception as e:
                    Logger.error("WelcomPageView.startGameFlow", f"Failed to stop welcome music: {e}")
            
            Logger.debug("WelcomPageView.startGameFlow", "Starting game flow with sequence controller")
            ...
            try:
                ...
                Logger.debug("WelcomPageView.startGameFlow", "Menu size saved", width=menu_size[0], height=menu_size[1])
            except Exception as e:
                Logger.error("WelcomPageView.startGameFlow", e)
            ...
            try:
                ...
                if not pre_fullscreen:
                    try:
                        ...
                        self.screen = screen
                        Logger.debug("WelcomPageView.startGameFlow", "Switched to FULLSCREEN for gameplay", width=full_size[0], height=full_size[1])
                    except Exception as e:
                        Logger.error("WelcomPageView.startGameFlow", e)
                else:
                    Logger.debug("WelcomPageView.startGameFlow", "Already in fullscreen", width=full_size[0], height=full_size[1])
            except Exception as e:
                Logger.error("WelcomPageView.startGameFlow", e)
            ...
            try:
                ...
                if self.current_user and self.user_progression:
                    ...
                    player = self.loadPlayerFromProgression(self.user_progression)
                ...
                try:
                    ...
                    manager_corrompu_config = asset_manager.getBossByBame("Manager Corrompu")
                    gros_bill_config = asset_manager.getBossByBame("Gros Bill")
                    ...
                    if chef_securite_config:
                        ...
                        Logger.warn("WelcomPageView.startGameFlow", "Chef de la Sécurité config not found, using fallback")
                    
                    Logger.debug("WelcomPageView.startGameFlow", "Bosses loaded from asset configurations")
                except Exception as e:
                    Logger.error("WelcomPageView.startGameFlow", f"Failed to load boss configs: {e}")
            ...
            while True:
                try:
                    ...
                    if result is None:
                        ...
                        Logger.debug("WelcomPageView.startGameFlow", "No result from stage", stage=current_stage)
                        break
                    
                    elif result == GameState.QUIT.value:
                        Logger.debug("WelcomPageView.startGameFlow", "Quit requested")
                        return GameState.QUIT.value
                    
                    elif result == GameState.LOGOUT.value:
                        Logger.debug("WelcomPageView.startGameFlow", "Logout requested")
                        return GameState.LOGOUT.value
                    
                    elif result == GameState.MAIN_MENU.value:
                        Logger.debug("WelcomPageView.startGameFlow", "Main menu requested")
                        break
                    
                    elif result == GameState.GAME_OVER.value:
                        Logger.debug("WelcomPageView.startGameFlow", "Game over")
                        return
                        ...
                        if not self.is_admin:
                            Logger.debug("WelcomPageView.startGameFlow", "Stage jump blocked: admin only")
                            continue
            ...
            try:
                if self.current_user:
                    self.savePlayerProgression(player, sequence_controller)
            except Exception as e:
                Logger.error("WelcomPageView.startGameFlow", f"Failed to save progression: {e}")
            ...
            try:
                if not self.music_playing:
                    try:
                        pygame.mixer.music.play(-1)  
                        self.music_playing = True
                        Logger.debug("WelcomPageView.startGameFlow", "Welcome music resumed on return to menu")
                    except Exception as e:
                        Logger.error("WelcomPageView.startGameFlow", f"Failed to resume welcome music: {e}")
            except Exception:
                pass
            ...
            try:
                if menu_size:
                    try:
                        ...
                        self.setWindowSize(menu_size[0], menu_size[1], menu_resizable if menu_resizable else pygame.RESIZABLE)
                        Logger.debug("WelcomPageView.startGameFlow", "Restored menu window size", width=menu_size[0], height=menu_size[1])
                    except Exception as e:
                        Logger.error("WelcomPageView.startGameFlow", e)
            except Exception:
                pass
