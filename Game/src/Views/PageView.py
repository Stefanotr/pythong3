        ...
        try:
            ...
            if not pygame.get_init():
                Logger.error(
                    ...
                    "PageView.__init__",
                    "Pygame not initialized. PageView requires pygame to be initialized before instantiation.",
                )
            ...
            try:
                ...
                if existing is not None:
                    ...
                    Logger.debug(
                        ...
                        "Reusing existing display surface",
            ...
            try:
                if (
                    self.backgroud_image
                    and isinstance(self.backgroud_image, str)
                    and os.path.exists(self.backgroud_image)
                ):
                    
                    try:
                        self._original_background = pygame.image.load(self.backgroud_image)
                        self.background = pygame.transform.scale(
                            self._original_background, (self.width, self.height)
                        )
                        Logger.debug(
                            "PageView.__init__",

                            "Background image loaded",
                            path=self.backgroud_image,
                        )
                    except Exception as e:
                        Logger.error("PageView.__init__", e)
                        self._original_background = None
                        ...
                        Logger.debug(
                            "PageView.__init__",
                            "Using default black background due to load failure",
                        )
                else:
                    self._original_background = None
                    self.background = pygame.Surface((self.width, self.height))
                    ...
                    Logger.debug(
                        "PageView.__init__",
                        "Using default black background (no image provided or file missing)",
    ...
    def rescaleBackground(self, new_width, new_height):
        try:
            ...
            if self._original_background is not None:
                self.background = pygame.transform.scale(
                    self._original_background, (new_width, new_height)
                )
                Logger.debug(

                    "PageView.rescaleBackground",
                    "Background rescaled",
                    width=new_width,
                    height=new_height,
                )
    ...
    def draw(self):
        try:
            ...
            try:
                ...
                Logger.debug(
                    "PageView.draw",
                    "Blit failed, rescaling background",
